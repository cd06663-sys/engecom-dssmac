...
app.post('/api/teams', async (req, res) => {
  ...
  try {
    const row = await sql1(
      'INSERT INTO teams (name, district_id, team_number, instructor) VALUES ($1, $2, $3, $4) RETURNING id',
      [name, district_id, team_number, instructor || null]
    );
    res.json({ id: row.id });
...
app.put('/api/teams/:id', async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE teams SET name=$1, team_number=$2, instructor=$3 WHERE id=$4',
      [name, team_number, instructor || null, req.params.id]
    );
