...
require('dns').setDefaultResultOrder('ipv4first'); // Railway não tem IPv6
...
const sql  = (q, v = []) => pool.query(q, v).then(r => r.rows);
const sql1 = (q, v = []) => pool.query(q, v).then(r => r.rows[0] || null);
...
const upload = multer({
  ...
  fileFilter: (_, f, cb) => {
    ...
    cb(null, ok.includes(path.extname(f.originalname).toLowerCase()));
  },
});
...
app.get('/api/districts', async (_, res) => {
  try { res.json(await sql('SELECT * FROM districts ORDER BY name')); }
  ...
  try {
    ...
    for (const s of sessions) {
      s.assignments = await sql(`
        ...
        WHERE a.session_id = $1 ORDER BY d.name, t.team_number
      `, [s.id]);
    }
    res.json(sessions);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
app.put('/api/sessions/:id', async (req, res) => {
  ...
  try {
    await pool.query(`
      UPDATE sessions SET title=$1, week=$2, month_year=$3, date=$4,
        time_start=$5, time_end=$6, description=$7, workload=$8, obra_vale=$9
      WHERE id=$10
...
app.get('/api/pdf/:sessionId/:teamId', async (req, res) => {
  try {
    ...
    const assignment = await sql1(
      'SELECT * FROM assignments WHERE session_id=$1 AND team_id=$2', [sessionId, teamId]
    );
    const employees = await sql(
      'SELECT * FROM employees WHERE team_id=$1 AND active=1 ORDER BY name', [teamId]
    );
...
app.get('/api/pdf/:sessionId/zip', async (req, res) => {
  ...
  try {
    ...
    const assignments = await sql(`
      SELECT a.*, t.id AS tid, t.name AS team_name, t.instructor, t.specialty, t.location,
    ...
    for (const a of assignments) {
      try {
        const employees = await sql(
          'SELECT * FROM employees WHERE team_id=$1 AND active=1 ORDER BY name', [a.tid]
        );
        const team = {
    ...
    archive.on('finish', () => tmpFiles.forEach(f => { try { fs.unlinkSync(f); } catch (_) {} }));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    for (const f of req.files) {
      ...
      const { error: upErr } = await supabase.storage
        .from('uploads').upload(fileName, f.buffer, { contentType: f.mimetype });
  ...
  try {
    ...
    if (row) {
      ...
      const cnt = await sql1(
        'SELECT COUNT(*)::int AS c FROM submissions WHERE assignment_id=$1', [row.assignment_id]
      );
      if (cnt.c === 0)
        await pool.query(
          "UPDATE assignments SET status='pending', submitted_at=NULL WHERE id=$1", [row.assignment_id]
        );
    }
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
...
app.post('/api/assignments/:id/reset', async (req, res) => {
  try {
    await pool.query(
      "UPDATE assignments SET status='pending', submitted_at=NULL WHERE id=$1", [req.params.id]
    );
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    const assignments = await sql(`
      SELECT a.*, s.title, s.date, s.week, s.month_year, s.time_start, s.time_end, s.description,
             (SELECT COUNT(*)::int FROM submissions WHERE assignment_id=a.id) AS sub_count
      FROM assignments a JOIN sessions s ON a.session_id = s.id
      WHERE a.team_id = $1 ORDER BY s.date DESC
    `, [req.params.teamId]);
